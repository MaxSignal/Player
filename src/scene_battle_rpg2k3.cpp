...
void Scene_Battle_Rpg2k3::CreateBattleCommandWindow() {
	...
	if (actor) {
		...
		for (const RPG::BattleCommand* command : bcmds) {
			...
			if (!IsEscapeAllowedFromActorCommand() && command->type == RPG::BattleCommand::Type_escape) {
...
bool Scene_Battle_Rpg2k3::IsEscapeAllowedFromOptionWindow() const {
	...
	return Scene_Battle::IsEscapeAllowed() && (Game_Battle::GetTurn() == 0)
		&& (first_strike || cond == RPG::System::BattleCondition_initiative || cond == RPG::System::BattleCondition_surround);
}

bool Scene_Battle_Rpg2k3::IsEscapeAllowedFromActorCommand() const {
	auto cond = Game_Battle::GetBattleCondition();

	return Scene_Battle::IsEscapeAllowed() && cond != RPG::System::BattleCondition_pincers;
}
